...
func ABVDistribution(db *gorm.DB, year string) ([]ABVBucket, error) {
	buckets := []ABVBucket{
	...
	for i, b := range buckets {
		...
		if year != "" {
			tx = tx.Where("strftime('%Y', checkins.checkin_at) = ?", year)
		}
		if b.Max >= 9999 {
			tx = tx.Where("beers.abv >= ?", b.Min)
		} else {
			tx = tx.Where("beers.abv >= ? AND beers.abv < ?", b.Min, b.Max)
		}
