...
func Beers(db *gorm.DB) ([]Beer, error) {
	...
	res := db.
		Table("checkins").
		Select("beers.name as name, AVG(checkins.rating_score) as avg_rating, count(checkins.id) as checkins, checkins.checkin_at, beers.ibu, beers.abv, breweries.name as brewery").
